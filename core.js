...
(function() {
  ...
  // Rate limiting: tối đa 5 lần sai trong 30s
  const _LFKEY = 'h5c_lf';
  function _checkLoginLock(errEl) {
    try {
      const [n, t] = JSON.parse(sessionStorage.getItem(_LFKEY) || '[0,0]');
      if (n >= 5 && Date.now() - t < 30000) {
        const wait = Math.ceil((30000 - (Date.now() - t)) / 1000);
        if (errEl) { errEl.textContent = `Quá nhiều lần thử sai. Vui lòng chờ ${wait}s.`; errEl.classList.add('show'); }
        return true;
      }
    } catch(e) {}
    return false;
  ...
  function _recordLoginFail() {
    try {
      let [n, t] = JSON.parse(sessionStorage.getItem(_LFKEY) || '[0,0]');
      if (Date.now() - t >= 30000) { n = 0; t = Date.now(); }
      sessionStorage.setItem(_LFKEY, JSON.stringify([n + 1, t || Date.now()]));
    } catch(e) {}
  }
  function _clearLoginFail() { sessionStorage.removeItem(_LFKEY); }
  ...
  function _unifiedHandleLogin() {
    ...
    if (_checkLoginLock(errEl)) return;
  ...
  window.navigateTo = function(page) {
    ...
    if (PAGE_MAP[page]) {
      ...
      setTimeout(function() { window.location.href = PAGE_MAP[page]; }, 110);
    }
  ...
  // ──────────────────────────────────────────────
  // 9b. MODAL HELPERS — ESC + backdrop close
  // ──────────────────────────────────────────────
  // ESC key closes topmost visible modal
  ...
  // ──────────────────────────────────────────────
  // 9c. OFFLINE INDICATOR
  // ──────────────────────────────────────────────
  function _setupOfflineIndicator() {
    const bar = document.createElement('div');
    bar.id = 'h5c-offline-bar';
    bar.textContent = '⚠️ Mất kết nối mạng — dữ liệu chưa được đồng bộ';
    bar.style.cssText = 'display:none;position:fixed;top:0;left:0;right:0;z-index:999999;background:#E65100;color:#fff;text-align:center;padding:6px 16px;font-size:12px;font-weight:600;letter-spacing:.3px;';
    document.body.appendChild(bar);
    window.addEventListener('offline', () => { bar.style.display = 'block'; });
    window.addEventListener('online',  () => { bar.style.display = 'none'; showToast('Đã kết nối lại mạng', 'success'); });
    if (!navigator.onLine) bar.style.display = 'block';
  }

  // ──────────────────────────────────────────────
  // 9d. SESSION EXTEND — gia hạn session khi user đang hoạt động
  // ──────────────────────────────────────────────
  function _setupSessionExtend() {
    let _lastExt = 0;
    function _extendIfActive() {
      const now = Date.now();
      if (now - _lastExt < 5 * 60 * 1000) return; // throttle: 1 lần / 5 phút
      _lastExt = now;
      try {
        const raw = localStorage.getItem(SESSION_KEY);
        if (!raw) return;
    ...
    ['click', 'keydown'].forEach(evt => document.addEventListener(evt, _extendIfActive, { passive: true }));
  }
  ...
  window.addEventListener('load', function() {
    ...
    requestAnimationFrame(function() { document.body.classList.add('h5c-ready'); });
